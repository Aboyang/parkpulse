...
// import { Crosshair, VolumeX, Volume2, X, Navigation as NavIcon, ArrowLeft, CornerDownLeft, CornerDownRight } from 'lucide-react';
...
// /**
//  * Fetch user's current position from the backend.
...
//     ? ` in ${step.distance >= 1000 ? (step.distance / 1000).toFixed(1) + ' km' : Math.round(step.distance) + ' m'}`
//     : '';
...
//  * start / end are both [lat, lng] — OSRM URL needs them flipped to lng,lat.
//  * Returns { pts: [lat, lng][], steps }
//  */
...
// /** Leaflet helper — re-centers the map whenever `center` changes. */
...
// const POLL_INTERVAL_MS = 3000; // how often to re-fetch position while navigating
...
//   const destination = carpark ? [carpark.latitude, carpark.longitude] : null; // [lat, lng] WGS84

//   // ── Step 1: get initial position, fetch route, then start polling ──────────
//   useEffect(() => {
...
//       if (!pos) {
//         // API failed — show denied state, nothing we can do without a position
...
//       // Fetch the route using the initial position as the start
//       const { pts, steps } = await fetchRoute(pos, destination);
//       if (cancelled) return;

//       const fullDist = pts.reduce(
...
//       // Poll for position updates every POLL_INTERVAL_MS
...
//   // ── Retry handler shown on the denied screen ───────────────────────────────
//   const handleRetry = async () => {
//     setLocationStatus('requesting');
//     const pos = await fetchUserPosition();
//     if (pos) {
//       setLocationStatus('granted');
//       // Re-trigger the main effect by resetting routeFetchedRef — easiest is
//       // to just call init logic inline here
//       const { pts, steps } = await fetchRoute(pos, destination);
//       const fullDist = pts.reduce(
//         (sum, p, i) => (i === 0 ? 0 : sum + getDistanceM(pts[i - 1], p)),
//         0,
...
//       if (pollRef.current) clearInterval(pollRef.current); // stop polling on arrival
//     }
//   }, [arrived]);
...
//             <Polyline positions={completedRoute} pathOptions={{ color: '#475569', weight: 5, opacity: 0.5 }} />
//           )}
//           {remainingRoute.length >= 2 && (
//             <Polyline positions={remainingRoute} pathOptions={{ color: '#2dd4bf', weight: 5, opacity: 1 }} />
//           )}
