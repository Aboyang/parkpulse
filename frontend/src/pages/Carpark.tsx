...
import { useCarparkRating } from '@/hooks/use-rating';
import { StatBlock } from '@/components/carpark/stat-block';
import { getTileUrl } from '@/utils/map';
import CarparkComments from '@/components/carpark/CarparkComments';
...
export default function Carpark() {
  ...
  const { data: ratingData, isLoading: ratingsLoading } = useCarparkRating(carpark?.id);
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {carpark.average_rating != null ? (
              <button
                ...
                className="flex items-center gap-1 bg-amber-500/20 px-2.5 py-1 rounded-lg shrink-0 cursor-pointer hover:ring-1 hover:ring-amber-400/50 transition-all"
              >
              ...
              <button
                onClick={() => setView('comments')}
                className="flex items-center gap-1 bg-slate-700/50 dark:bg-slate-200/50 px-2.5 py-1 rounded-lg shrink-0 cursor-pointer hover:ring-1 hover:ring-slate-400/50 dark:hover:ring-slate-400/50 transition-all"
              >
