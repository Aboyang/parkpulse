...
export default function Home() {
  ...
  const handleSearch = async () => {
    setSearching(true);
    // Always re-fetch live GPS so user's real location is always fresh
    const freshPos = await getCurrentPosition();
    
    // Determine destination to search
    let searchDest = destination;
    if (!selectedCoords && freshPos) {
      // If user didn't pick a destination, use fresh location
      searchDest = await reverseGeocode(freshPos.lat, freshPos.lng);
    }
    
    // Use fresh GPS for "near me" searches; use selectedCoords only if user explicitly picked a destination
    ...
    const params = new URLSearchParams({
      ...
      radius: radius,
      t: Date.now(),
      // Always pass fresh GPS as the user's real position for distance sorting
      ...
      <div className="px-5 pt-14 pb-8 max-w-lg mx-auto">
        ...
        <motion.div
          ...
          <div>
            ...
            <SearchBar
              value={destination}
              onChange={(v) => {setDestination(v);setSelectedCoords(null);}}
            ...
            {!gpsLoading && selectedCoords && !destination.trim() === false &&
