...
class CarparkAvailabilityService {
  ...
  async getGeocode(address: string): Promise<{ formattedAddress: string; latitude: number; longitude: number }> {
  ...
  async fetchAllCarparkAvailability(): Promise<CarparkAvailabilityApiEntry[]> {
    ...
    try {
      ...
      console.error("Error fetching carpark availability:", (error as { response?: { data: unknown }; message: string }).response?.data || (error as Error).message);
      throw error;
    }
  }

  async fetchCarparkAvailabilityById(carparkId: string): Promise<{ lots_available: string; total_lots: string } | undefined> {
  ...
  async fetchEnrichedCarparksAtCoords(
    latitude: number,
    longitude: number,
    radius: number,
    evCharging: boolean
    ...
    const enrichedCarparks = await Promise.all(
      nearbyCarparks.map(async ({ carpark, dist }) => {
